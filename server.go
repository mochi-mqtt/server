...
type Options struct {
	...
	// PoolSize specifies the number of workers which will be used to dispatch incoming
	// publish messages to outbound client message queues. It doesn't need to be big, as it
	// will be mostly limited by the number of cores you have.
	PoolSize uint64
...
type Server struct {
	...
	pool      *Pool                // a fixed size worker pool for processing inbound and outbound messages
...
func (s *Server) publishToClient(cl *Client, sub packets.Subscription, pk packets.Packet) (packets.Packet, error) {
	...
	if atomic.LoadInt32(&cl.State.outboundQty) >= s.Options.Capabilities.MaximumClientWritesPending {
		return out, packets.ErrPendingClientWritesExceeded
	}

	cl.State.outbound <- out
